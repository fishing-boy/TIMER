...
void TIM_INIT(void)
{
	...
	TIM_ClearITPendingBit(TIM1,TIM_IT_Trigger);//清除更新中断请求位
	TIM_ITConfig(TIM1,TIM_IT_Trigger,ENABLE); //允许定时器1更新中断
	TIM_Cmd(TIM1,ENABLE); //使能定时器1
	
	NVIC_InitStructure.NVIC_IRQChannel=TIM1_TRG_COM_TIM11_IRQn; //定时器1中断
...
{
	if(TIM_GetITStatus(TIM1,TIM_IT_Trigger)==SET) //溢出中断
	{
		TIM_ClearITPendingBit(TIM1,TIM_IT_Trigger);  //清除中断标志位
		printf("TIM1 Trigger EVENT PASS\r\n");
