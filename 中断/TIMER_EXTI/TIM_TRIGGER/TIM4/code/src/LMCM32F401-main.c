...
void TIM_INIT(void)
{
	...
	TIM_ClearITPendingBit(TIM4,TIM_IT_Trigger);//清除更新中断请求位
	TIM_ITConfig(TIM4,TIM_IT_Trigger,ENABLE); //允许定时器1更新中断
	TIM_Cmd(TIM4,ENABLE); //使能定时器1
	
	NVIC_InitStructure.NVIC_IRQChannel=TIM4_IRQn; //定时器1中断
...
{
	if(TIM_GetITStatus(TIM4,TIM_IT_Trigger)==SET) //溢出中断
	{
		TIM_ClearITPendingBit(TIM4,TIM_IT_Trigger);  //清除中断标志位
		printf("TIM4 Trigger EVENT PASS\r\n");
