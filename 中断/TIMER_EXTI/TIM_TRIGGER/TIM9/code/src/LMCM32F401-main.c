...
void TIM_INIT(void)
{
	...
	TIM_ClearITPendingBit(TIM9,TIM_IT_Trigger);//清除更新中断请求位
	TIM_ITConfig(TIM9,TIM_IT_Trigger,ENABLE); //允许定时器1更新中断
...
{
	if(TIM_GetITStatus(TIM9,TIM_IT_Trigger)==SET) //溢出中断
	{
		TIM_ClearITPendingBit(TIM9,TIM_IT_Trigger);  //清除中断标志位
		printf("TIM9 Trigger EVENT PASS\r\n");
