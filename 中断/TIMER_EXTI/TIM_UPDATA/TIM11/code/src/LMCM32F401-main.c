...
void TIM_INIT(void)
{
	...
	TIM_TimeBaseInit(TIM11,&TIM_TimeBaseInitStructure);//初始化TIM11
	
	TIM_ClearITPendingBit(TIM11,TIM_IT_Update);//清除更新中断请求位
	TIM_ITConfig(TIM11,TIM_IT_Update,ENABLE); //允许定时器1更新中断
...
void TIM1_TRG_COM_TIM11_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM11,TIM_IT_Update)==SET) //溢出中断
	{
		...
		printf("TIM11 UPDATA EVENT PASS\r\n");
		TIM_Cmd(TIM11,DISABLE); //失能定时器2
		EVENT_FLAG = 1;
	}
}
