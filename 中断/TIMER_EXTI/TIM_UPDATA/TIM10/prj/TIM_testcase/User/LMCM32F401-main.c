...
void TIM_INIT(void)
{
	...
	TIM_ClearITPendingBit(TIM10,TIM_IT_Update);//清除更新中断请求位
	TIM_ITConfig(TIM10,TIM_IT_Update,ENABLE); //允许定时器1更新中断
...
void TIM1_UP_TIM10_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM10,TIM_IT_Update)==SET) //溢出中断
	{
		...
		printf("TIM10 UPDATA EVENT PASS\r\n");
		TIM_Cmd(TIM10,DISABLE); //失能定时器2
		EVENT_FLAG = 1;
	}
}
