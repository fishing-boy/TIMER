...
void TIM_INIT(void)
{
	...
	TIM_ClearITPendingBit(TIM1,TIM_IT_COM);//清除更新中断请求位
	TIM_ITConfig(TIM1,TIM_IT_COM,ENABLE); //允许定时器1更新中断
...
{
	if(TIM_GetITStatus(TIM1,TIM_IT_COM)==SET) //溢出中断
	{
		TIM_ClearITPendingBit(TIM1,TIM_IT_COM);  //清除中断标志位
		printf("TIM1 COM EVENT PASS\r\n");
